...
3. **Run the daily loop** —
   ...
   - **A link target git does not have is not a target the wiki has.** The mirror image: a target that exists in your working tree but is untracked (an allowlist `.gitignore` that never re-included its directory is the usual cause) reports as `untracked`. It resolves for you and for nobody else, and search and the graph — both built from `git ls-files` — cannot see it either. Advisory by default so an upgrade never blocks an existing wiki; `advisory_budgets.untracked` gates it.
